...
import Glas.Test.ColoredLogger;
import std;


void testLogger(std::shared_ptr<Glas::Test::ColoredLogger> logger) {
...
{
    try {
        auto logger = Glas::Test::ColoredLogger::create("ColoredLogger");
        testLogger(logger);

        logger->changeInfoEntryStyle();
        testLogger(logger);
